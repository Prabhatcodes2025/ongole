...
export async function GET(request:NextRequest){
  const next=safeReturnPath(request.nextUrl.searchParams.get("next"));const code=request.nextUrl.searchParams.get("code");const tokenHash=request.nextUrl.searchParams.get("token_hash");const type=request.nextUrl.searchParams.get("type") as EmailOtpType|null;const intent=request.nextUrl.searchParams.get("intent");const termsVersion=request.nextUrl.searchParams.get("termsVersion");const supabase=await createSupabaseServerClient();
