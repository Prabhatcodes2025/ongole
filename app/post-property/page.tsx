...
export default async function PostPropertyPage(){
  if(env.isSupabaseConfigured){const supabase=await createSupabaseServerClient();const{data:auth}=await supabase.auth.getUser();
    if(auth.user){const{data:permission,error}=await supabase.rpc("check_property_posting_permission");if(error||!(permission as {allowed?:boolean}|null)?.allowed)return <main id="main"><section className="section shell"><p className="form-message error" role="alert">{POSTING_ENTITLEMENT_MESSAGE}</p></section></main>}}
