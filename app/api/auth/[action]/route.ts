...
async function startGoogleOAuth(request:NextRequest,data:Record<string,unknown>){
  ...
  const callback=new URL("/auth/callback",request.nextUrl.origin);callback.searchParams.set("next",returnTo);if(data.keepSignedIn==="accepted")callback.searchParams.set("keep","1");if(accountType){callback.searchParams.set("intent",accountType);callback.searchParams.set("termsVersion",termsVersion)}
