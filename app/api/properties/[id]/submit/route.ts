...
export async function POST(request:NextRequest,{params}:{params:Promise<{id:string}>}){
  ...
  const{error}=await supabase.rpc("submit_property_for_review",{target_property:id});if(error)return NextResponse.json({error:error.message==="PROPERTY_POSTING_LIMIT_REACHED"?POSTING_ENTITLEMENT_MESSAGE:"The property is incomplete or cannot be submitted from its current status."},{status:409});
