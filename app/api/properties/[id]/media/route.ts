...
export async function POST(request:NextRequest,{params}:{params:Promise<{id:string}>}){
  ...
  const supabase=await createSupabaseServerClient();const{data:auth}=await supabase.auth.getUser();if(!auth.user)return NextResponse.redirect(new URL(`/login?returnTo=/dashboard/properties/${id}`,request.url),303);if(!auth.user.email_confirmed_at)return NextResponse.json({error:"Verify your email address before uploading property images."},{status:403});const{data:property}=await supabase.from("properties").select("id,status").eq("id",id).eq("owner_id",auth.user.id).single();if(!property||!["draft","changes_requested"].includes(property.status))return NextResponse.json({error:"This property cannot accept uploads."},{status:403});
  const form=await request.formData();const files=form.getAll("image").filter((item):item is File=>item instanceof File&&item.size>0);if(!files.length||files.length>6||files.some((file)=>file.size>MAX_SOURCE_BYTES||!ALLOWED_INPUT_TYPES.has(file.type)))return NextResponse.redirect(dashboardUrl(request,id,"invalid"),303);
  const{count}=await supabase.from("property_media").select("id",{count:"exact",head:true}).eq("property_id",id).eq("media_type","image");const limit=6;if((count??0)+files.length>limit)return NextResponse.redirect(dashboardUrl(request,id,"limit"),303);
  const storedPaths:string[]=[],storedIds:string[]=[];
  try{
    const{default:sharp}=await import("sharp");
    for(const[index,file]of files.entries()){
      const source=Buffer.from(await file.arrayBuffer()),baseImage=sharp(source,{failOn:"warning",limitInputPixels:40_000_000}).rotate().resize({width:1200,height:1200,fit:"inside",withoutEnlargement:true}),dimensions=await baseImage.clone().metadata();if(!dimensions.width||!dimensions.height)throw new Error("Image dimensions unavailable");
      const watermarkWidth=Math.max(360,Math.min(900,Math.round(dimensions.width*.72))),watermarkHeight=Math.max(44,Math.round(watermarkWidth*.09)),label="www.ongoleproperty.com Call 7788998459";const watermark=Buffer.from(`<svg width="${watermarkWidth}" height="${watermarkHeight}" xmlns="http://www.w3.org/2000/svg"><rect width="100%" height="100%" rx="8" fill="#111827" fill-opacity="0.58"/><text x="50%" y="54%" text-anchor="middle" dominant-baseline="middle" font-family="Arial, sans-serif" font-size="${Math.round(watermarkHeight*.3)}" font-weight="700" fill="#ffffff">${label}</text></svg>`);
      const displayImage=isPg?baseImage:baseImage.composite([{input:watermark,gravity:"southeast"}]);
      const processed=await displayImage.webp({quality:isPg?88:82,effort:5}).toBuffer(),output=await sharp(processed).metadata(),thumbnail=await sharp(processed).resize({width:400,height:225,fit:"cover",position:"centre",withoutEnlargement:true}).webp({quality:isPg?84:78,effort:5}).toBuffer(),thumbnailOutput=await sharp(thumbnail).metadata(),mediaId=crypto.randomUUID(),path=`${auth.user.id}/${id}/${mediaId}.webp`,thumbnailPath=`${auth.user.id}/${id}/${mediaId}-thumb.webp`;
