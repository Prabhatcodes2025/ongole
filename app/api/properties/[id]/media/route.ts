...
export async function POST(request:NextRequest,{params}:{params:Promise<{id:string}>}){
  ...
  try{
    ...
    for(const[index,file]of files.entries()){
      ...
      const watermarkWidth=Math.max(360,Math.min(900,Math.round(dimensions.width*.6))),watermarkHeight=Math.max(44,Math.round(watermarkWidth*.09)),label="7788998459 , www.ongoleproperty.com";const watermark=Buffer.from(`<svg width="${watermarkWidth}" height="${watermarkHeight}" xmlns="http://www.w3.org/2000/svg"><rect width="100%" height="100%" rx="8" fill="#111827" fill-opacity="0.58"/><text x="50%" y="54%" text-anchor="middle" dominant-baseline="middle" font-family="Arial, sans-serif" font-size="${Math.round(watermarkHeight*.3)}" font-weight="700" fill="#ffffff">${label}</text></svg>`);
