...
export async function POST(request:NextRequest,{params}:{params:Promise<{id:string;mediaId:string}>}){
  ...
  const {data:media}=await supabase.from("property_media").select("id,storage_path,is_cover,sort_order").eq("id",mediaId).eq("property_id",id).maybeSingle();if(!media)return NextResponse.json({error:"Image not found."},{status:404});
  const payload=await requestData(request);const action=typeof payload.action==="string"?payload.action:"";
  if(action==="remove"){
    const {error:storageError}=await supabase.storage.from("property-media").remove([media.storage_path]);if(storageError)return NextResponse.json({error:"The image file could not be removed."},{status:500});
