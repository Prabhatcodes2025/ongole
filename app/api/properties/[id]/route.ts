...
export async function POST(request:NextRequest,{params}:{params:Promise<{id:string}>}){
  ...
  const value=parsed.data;const existing=property.details&&typeof property.details==="object"&&!Array.isArray(property.details)?property.details as Record<string,unknown>:{};const type=typeof existing.property_type_slug==="string"?existing.property_type_slug:"";const applicable=applicablePropertyDetails(value,property.transaction_type,type);if(!applicable.valid)return NextResponse.json({error:"Complete the fields applicable to this property type.",fields:applicable.errors},{status:400});if(value.youtubeUrl&& !youtubeVideoId(value.youtubeUrl))return NextResponse.json({error:"Enter a supported YouTube video URL."},{status:400});
  const details={...existing};for(const key of ["property_age","floor","total_floors","bedrooms","bathrooms","facing","crop_type","soil_type","amount_basis","fencing","electricity_connection","road_access","parking","power_backup","generator","security","cctv","lift","fire_safety","waste_management","balcony","pooja_room","store_room","servant_room","gas_pipeline","google_maps_url","youtube_url","nearby_places"])delete details[key];Object.assign(details,applicable.details);
