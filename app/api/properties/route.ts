...
import {applicablePropertyDetails,propertyDescriptionIsPublicSafe,propertyTitleIsProductionSafe,propertyTypeSlug as normalizePropertyTypeSlug,rentNearbyErrors} from "@/src/lib/properties/validation";
import {youtubeVideoId} from "@/src/lib/youtube";
import {POSTING_ENTITLEMENT_MESSAGE} from "@/src/lib/properties/posting-entitlement";

const schema = z.object({ transactionType: z.enum(["sale","rent","lease"]), category: z.string().min(2).max(60), propertyType: z.string().trim().min(2).max(80), title: z.string().trim().min(10).max(120).refine(propertyTitleIsProductionSafe,"Remove placeholder, script or database-command content from the title."), description: z.string().trim().min(20,"Description must be at least 20 characters.").max(250,"Description must not exceed 250 characters.").refine(propertyDescriptionIsPublicSafe,"Remove phone numbers, email addresses, WhatsApp/social handles, links, scripts or database commands from the description."), locality: z.string().trim().min(2).max(120), city: z.string().trim().min(2).max(120), district: z.string().trim().min(2).max(120), state: z.string().trim().min(2).max(120), price: z.coerce.number().nonnegative(), areaValue: z.coerce.number().positive(), areaUnit: z.enum(["gadi","sq_ft","sq_yd","sq_m","acre","cent","gunta","hectare"]), declaration: z.literal("accepted"),draftKey:z.string().uuid() }).passthrough();
...
export async function POST(request: NextRequest) {
  ...
  if(permissionError||!permissionResult?.allowed)return NextResponse.json({error:"LISTING_LIMIT_REACHED",message:POSTING_ENTITLEMENT_MESSAGE},{status:409});
  const value = parsed.data;const applicable=applicablePropertyDetails(value,value.transactionType,value.propertyType),nearbyErrors=rentNearbyErrors(value,value.transactionType);if(!applicable.valid||Object.keys(nearbyErrors).length)return NextResponse.json({error:"Complete the fields applicable to this property type.",fields:{...applicable.errors,...nearbyErrors}},{status:400});const youtubeUrl=typeof value.youtubeUrl==="string"?value.youtubeUrl.trim():"";if(youtubeUrl&&!youtubeVideoId(youtubeUrl))return NextResponse.json({error:"Enter a supported YouTube video URL."},{status:400});const baseSlug = propertySlug({ title: value.title, transactionType: value.transactionType, locality: value.locality, city: value.city });
  ...
  // Sale retains the strict legacy guard: if(!category||!propertyType)return.
  if(!category||(!propertyType&&value.transactionType==="sale"))return NextResponse.json({error:"Choose an active property category and type."},{status:400});
  ...
  // Known catalog values continue to use property_type_id:propertyType.id.
  const insertPayload={ owner_id: auth.user.id, category_id:category.id, property_type_id:propertyType?.id||null, transaction_type: value.transactionType, title: value.title, description: value.description, slug: `${baseSlug}-${crypto.randomUUID()}`, status: "draft" as const, locality_text: value.locality, city_text: value.city, district_text: value.district, state_text: value.state, price_inr: value.price, area_value: value.areaValue, area_unit: value.areaUnit, details: { ...applicable.details,category:value.category,property_type:propertyType?.name||value.propertyType,property_type_slug:propertyType?.slug||propertyTypeSlug,client_draft_key:value.draftKey,declaration_accepted_at:new Date().toISOString() } };
