...
export async function POST(request: NextRequest) {
  ...
  const primaryMobile=parsed.data.mobile||parsed.data.foreignMobile||null;
  const { data, error } = await supabase.from("enquiries").insert({ property_id: propertyId, name: parsed.data.name, mobile: primaryMobile, email: parsed.data.email || null, message: parsed.data.message, source: "website", attribution: { property_reference: parsed.data.propertyReference || null,enquiry_type:parsed.data.enquiryType||null,property_requirement:parsed.data.propertyRequirement||null,is_foreign_mobile:parsed.data.isForeign==="true",indian_mobile:parsed.data.mobile||null,foreign_mobile:parsed.data.foreignMobile||null,country_code:parsed.data.isForeign==="true"?parsed.data.countryCode:null,communication_consent:parsed.data.consent==="true"?{accepted:true,accepted_at:new Date().toISOString(),channels:["sms","whatsapp","email"]}:null }, privacy_safe_ip_hash: null, user_agent: request.headers.get("user-agent")?.slice(0, 500) || null }).select("reference_no").single();
