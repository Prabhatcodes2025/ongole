...
export function PgForm({defaults}:{defaults?:Record<string,unknown>}){
  ...
  return <form className="submission-form compact-form" action={defaults?.id?`/api/pg/${defaults.id}`:"/api/pg"} method="post">
    ...
    <label className="wide">Description <span>(maximum 250 words; no contact details, links or advertisements)</span><textarea name="description" minLength={40} rows={6} defaultValue={String(defaults?.description||"")}/></label>
    ...
    <button className="button" type="submit">{defaults?.id?"Save changes":"Submit Your PG"}</button>
  </form>;
}
