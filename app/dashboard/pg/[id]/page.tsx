...
const notices:Record<string,string>={created:"PG draft created.",updated:"PG details saved.",duplicated:"PG draft duplicated.","duplicate-failed":"The PG could not be duplicated. Your original listing was not changed.",submitted:"PG submitted for review.","consent-required":"Save the PG information and confirm the mandatory authorization and communication consent before submitting.","room-added":"Room type added.","room-removed":"Room type removed.",uploaded:"Image uploaded.",remove:"Image removed.",cover:"Cover image updated.",up:"Image moved earlier.",down:"Image moved later."};
...
export default async function ManagePgPage({params,searchParams}:{params:Promise<{id:string}>;searchParams:Promise<{notice?:string;media?:string}>}){
  ...
  return <main id="main" className="portal-page"><div className="shell">
    ...
    <div className="portal-title"><div><p className="eyebrow">{property.reference_no}</p><h1>{pg.pg_name}</h1><p><span className={`status status-${property.status}`}>{property.status.replaceAll("_"," ")}</span> · Updated {new Date(property.updated_at).toLocaleDateString("en-IN")}</p></div><div className="portal-actions"><Link className="button button-light" href={`/dashboard/pg/${id}/preview`}>Preview</Link>{editable&&hasConsent&&<form action={`/api/pg/${id}/submit`} method="post"><button className="button">Submit for review</button></form>}</div></div>
    {notice&&<p className={`form-message ${query.notice==="consent-required"?"error":"success"}`}>{notice}</p>}{editable&&!hasConsent&&<p className="form-message error">Confirm the mandatory consent and save the PG information to enable submission for review.</p>}
    <div className="review-layout owner-manage"><div>
      ...
      <section className="portal-section"><div className="section-heading-row"><h2>Room types</h2><span>{rooms?.length||0}</span></div>
        ...
        {editable&&<form className="submission-form compact-form room-form" action={`/api/pg/${id}/rooms`} method="post"><label>Room name<input required name="name" placeholder="Standard room"/></label><label>Sharing<select name="sharing_type"><option value="single">Single</option><option value="double">Double</option><option value="triple">Triple</option><option value="four_sharing">Four sharing</option></select></label><label>Capacity<input required name="capacity" type="number" min="1"/></label><label>Available beds<input required name="available_beds" type="number" min="0"/></label><label>Monthly rent<input required name="monthly_rent" type="number" min="0"/></label><button className="button secondary">Add room</button></form>}
      </section>
      <section className="portal-section"><div className="section-heading-row"><h2>Images</h2><span>{media.length}/6</span></div>
        {editable&&<form className="media-upload" action={`/api/properties/${pg.property_id}/media?context=pg&pgId=${id}`} method="post" encType="multipart/form-data"><label>Upload up to 6 images<input required multiple type="file" name="image" accept="image/jpeg,image/png,image/webp"/></label><p>Images are resized, optimized and converted to high-quality WebP without an OngoleProperty watermark.</p><button className="button secondary">Process &amp; upload</button></form>}
