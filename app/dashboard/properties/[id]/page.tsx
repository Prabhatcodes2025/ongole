...
export default async function ManagePropertyPage({params,searchParams}:{params:Promise<{id:string}>;searchParams:Promise<{media?:string;notice?:string}>}){
  ...
  return <main id="main" className="portal-page"><div className="shell">
    ...
    <div className="portal-title"><div><p className="eyebrow">{property.reference_no}</p><h1>{property.title}</h1><p><span className={`status status-${property.status}`}>{property.status.replaceAll("_"," ")}</span> · Updated {new Date(property.updated_at).toLocaleDateString("en-IN")}</p>{property.status==="expired"&&<p className="form-message">Status: EXPIRED. Contact OngoleProperty.com for offline renewal; renewal keeps this same property record.</p>}</div><div className="portal-actions"><Link className="button button-light" href={`/dashboard/properties/${id}/preview`}>Preview</Link>{editable&&<AsyncSubmitProperty action={`/api/properties/${id}/submit`} successUrl={`/dashboard/properties/${id}?notice=submitted`}/>}</div></div>
