...
export default async function OwnerProperties({searchParams}:{searchParams:Promise<{status?:string;q?:string}>}){
  const params=await searchParams;const{supabase,user}=await getDashboardAuth();if(!user)redirect("/login?returnTo=/dashboard/properties");
  let query=supabase.from("properties").select("id,reference_no,title,status,locality_text,city_text,price_inr,updated_at,details").eq("owner_id",user.id).is("deleted_at",null).order("updated_at",{ascending:false});
