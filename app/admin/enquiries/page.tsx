...
export default async function EnquiriesPage({searchParams}:{searchParams:Promise<{status?:string;priority?:string;q?:string;page?:string}>}){
  ...
  const {data,count,error}=await query;if(error){logEvent("error","admin.enquiries_query_failed",{code:error.code});return <QueryFailure/>}const pages=Math.max(1,Math.ceil((count||0)/size));
...
function QueryFailure(){return <main id="main" className="portal-page admin-portal"><div className="shell"><AdminNavigation/><div className="config-warning" role="alert"><h1>Enquiries could not be loaded</h1><p>Retry this page. If the problem continues, contact support with the request ID.</p><Link className="button" href="/admin/enquiries">Retry</Link></div></div></main>}
