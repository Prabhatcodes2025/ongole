...
import {FloatingWhatsAppControls} from "@/src/components/floating-whatsapp-controls";
...
export default function RootLayout({ children }: Readonly<{ children: React.ReactNode }>) {
  ...
  return <html lang="en-IN"><body className={`${geist.variable} ${lora.variable}`}><script type="application/ld+json" dangerouslySetInnerHTML={{__html:JSON.stringify(structuredData).replace(/</g,"\\u003c")}}/><Analytics measurementId={process.env.NEXT_PUBLIC_GA_MEASUREMENT_ID}/><NumericInputGuard/><SiteHeader />{children}<SiteFooter /><FloatingWhatsAppControls/></body></html>;
}
