...
test("registration supports Terms evidence, Google OAuth and safe validation logging",async()=>{
  const[route,form,migration]=await Promise.all([read("../app/api/auth/[action]/route.ts"),read("../src/components/registration-form.tsx"),read("../supabase/migrations/202608130001_targeted_auth_terms_fix.sql")]);
  assert.match(route,/signInWithOAuth\(\{provider:"google"/);assert.match(route,/terms_accepted_at/);assert.match(route,/registration_validation_failed/);assert.match(form,/Terms &amp; Conditions/);assert.match(migration,/claim_new_google_owner/);assert.match(migration,/insert into public\.agents/);assert.doesNotMatch(route,/service.role/i);
});
