...
const sequentialDigits=new Set(["1234567890","9876543210"]);

export function normalizeMobile(value:string){return value.trim()}

export function isValidIndianMobile(value:string){
  const mobile=normalizeMobile(value);
  return indianMobilePattern.test(mobile)&&!repeatedDigits.test(mobile)&&!repeatedPair.test(mobile)&&!repeatedBlock.test(mobile)&&!sequentialDigits.has(mobile);
}
