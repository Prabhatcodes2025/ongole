...
export type PublicPg={
  ...
  address:string;locality:string;city:string;district:string;state:string;latitude:number|null;longitude:number|null;
  rent:number;deposit:number|null;capacity:number|null;amenities:string[];rules:string[];videos:string[];
  verified:boolean;featured:boolean;pinned:boolean;publishedAt:string|null;media:{id:string;url:string;alt:string}[];
  rooms:{id:string;name:string;sharing:string;capacity:number;availableBeds:number;rent:number;deposit:number|null}[];
};
export type PgFilters={q?:string;city?:string;locality?:string;min?:number;max?:number;gender?:string;sharing?:string;amenities?:string[];available?:boolean;page:number;pageSize:number};
...
async function mapRows(rows:Row[]):Promise<PublicPg[]>{
  ...
  return rows.map((row)=>{
    const property=nested(row.properties)||{};const rooms=Array.isArray(row.pg_room_types)?row.pg_room_types as Row[]:[];
    return{id:String(row.id),propertyId:String(row.property_id),reference:String(property.reference_no),slug:String(property.slug),name:String(row.pg_name),category:String(row.category),description:String(property.description||""),address:String(row.address_line||""),locality:String(property.locality_text),city:String(property.city_text),district:String(property.district_text),state:String(property.state_text),latitude:property.latitude==null?null:Number(property.latitude),longitude:property.longitude==null?null:Number(property.longitude),rent:Number(row.rent_per_bed||0),deposit:row.security_deposit==null?null:Number(row.security_deposit),capacity:row.capacity==null?null:Number(row.capacity),amenities:strings(row.amenities),rules:strings(row.house_rules),videos:strings(row.video_urls),verified:Boolean(property.is_verified),featured:Boolean(property.is_featured),pinned:Boolean(property.is_pinned),publishedAt:typeof property.published_at==="string"?property.published_at:null,rooms:rooms.map((room)=>({id:String(room.id),name:String(room.name),sharing:String(room.sharing_type),capacity:Number(room.capacity),availableBeds:Number(room.available_beds),rent:Number(room.monthly_rent),deposit:room.security_deposit==null?null:Number(room.security_deposit)})),media:mediaRows.map((item,index)=>({item,url:signed?.[index]?.signedUrl||""})).filter(({item,url})=>String(item.property_id)===String(row.property_id)&&url).slice(0,12).map(({item,url})=>({id:String(item.id),url,alt:String(item.alt_text||row.pg_name)}))};
  });
}

const SELECT="id,property_id,pg_name,category,rent_per_bed,security_deposit,capacity,address_line,amenities,house_rules,video_urls,properties!inner(reference_no,slug,description,status,locality_text,city_text,district_text,state_text,latitude,longitude,is_verified,is_featured,is_pinned,published_at,deleted_at),pg_room_types(id,name,sharing_type,capacity,available_beds,monthly_rent,security_deposit,sort_order)";
...
export async function listPublicPgs(filters:PgFilters){
  ...
  if(filters.sharing||filters.available){
    let roomQuery=service.from("pg_room_types").select("pg_listing_id");
    if(filters.sharing)roomQuery=roomQuery.eq("sharing_type",filters.sharing);
  ...
  let query=service.from("pg_listings").select(SELECT,{count:"exact"}).eq("properties.status","published").is("properties.deleted_at",null);
  ...
  const {data,count,error}=await query.order("is_pinned",{referencedTable:"properties",ascending:false}).order("published_at",{referencedTable:"properties",ascending:false}).range(start,start+filters.pageSize-1);
...
export async function getPublicPg(slug:string){
  ...
  const {data}=await service.from("pg_listings").select(SELECT).eq("properties.status","published").is("properties.deleted_at",null).eq("properties.slug",safe).maybeSingle();
