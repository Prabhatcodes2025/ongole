...
export const DEV_JV_CATEGORY = "dev-jv";
export const DEV_JV_LABEL = "Property for Development / Joint Venture";
export function isDevelopmentJv(property: {transactionType: string; categorySlug?: string}) {
  return property.transactionType === "sale" && property.categorySlug === DEV_JV_CATEGORY;
}
...
export const propertyCategories = [
  ...
  { value: DEV_JV_CATEGORY, label: DEV_JV_LABEL, types: ["open-plot"] },
] as const;
...
export function typesFromCatalog(catalog:PropertyCatalogData,category?:string){return category?catalog.categories.find((item)=>item.value===category)?.types||[]:catalog.categories.filter((item)=>item.value!==DEV_JV_CATEGORY).flatMap((item)=>item.types)}
