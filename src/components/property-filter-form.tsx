...
export function PropertyFilterForm({filters,catalog=fallbackPropertyCatalog}:{filters:PropertyFilters;catalog?:PropertyCatalogData}){
  ...
  const form=<form className="filter-form" action="/properties" method="get"><div className="filter-form-head"><h2>Filter properties</h2><button type="button" onClick={()=>setOpen(false)} aria-label="Close filters"><X/></button></div>
    ...
    <fieldset><legend>Budget (₹)</legend><div className="field-pair"><input name="minPrice" defaultValue={filters.minPrice} type="number" min="0" placeholder="Minimum" aria-label="Minimum price"/><input name="maxPrice" defaultValue={filters.maxPrice} type="number" min="0" placeholder="Maximum" aria-label="Maximum price"/></div></fieldset>
    <fieldset><legend>Area (square feet equivalent)</legend><div className="field-pair"><input name="minArea" defaultValue={filters.minArea} type="number" min="0" placeholder="Minimum" aria-label="Minimum area in square feet"/><input name="maxArea" defaultValue={filters.maxArea} type="number" min="0" placeholder="Maximum" aria-label="Maximum area in square feet"/></div></fieldset>
