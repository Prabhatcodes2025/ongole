...
import {AGENT_DRAFT_KEY} from "@/src/components/agent-application-prefill";

const accountTypes = [
  // Copy contract: the agent profile remains pending through administrator review.
  ["buyer", "Buyer"],
...
export function RegistrationForm({ initialAccountType, returnTo,serverErrorFields=[] }: { initialAccountType: string; returnTo: string;serverErrorFields?:RegistrationField[] }) {
  ...
  const[agentDraft,setAgentDraft]=useState({yearsExperience:"",officeAddress:"",workingTowns:"",specializations:"",about:""});
  ...
  useEffect(()=>{if(initialAccountType!=="agent")return;const timer=window.setTimeout(()=>{try{const parsed=JSON.parse(localStorage.getItem(AGENT_DRAFT_KEY)||"{}");setAgentDraft(current=>Object.fromEntries(Object.keys(current).map(key=>[key,typeof parsed[key]==="string"?parsed[key]:""])) as typeof current)}catch{/* Ignore unavailable or invalid browser-only draft data. */}},0);return()=>window.clearTimeout(timer)},[initialAccountType]);
  ...
  return <>
    ...
    <GoogleOAuthButton returnTo={returnTo} accountType={accountType} accountLabel={accountLabel} agentApplication={isAgent?agentDraft:undefined}/>
  ...
  <form action="/api/auth/register" method="post" noValidate onSubmit={(event)=>{const values=Object.fromEntries(new FormData(event.currentTarget));const next=validateRegistrationFields(values);if(Object.keys(next).length){event.preventDefault();setErrors(next);const first=Object.keys(next)[0];event.currentTarget.querySelector<HTMLElement>(`[name="${first}"]`)?.focus()}else setErrors({})}}>
    ...
    {isAgent&&<fieldset className="agent-registration-fields"><legend>Professional profile restored from this browser</legend><label>Years of experience <span>(optional)</span><input name="yearsExperience" type="number" min="0" max="80" inputMode="numeric" value={agentDraft.yearsExperience} onChange={event=>setAgentDraft({...agentDraft,yearsExperience:event.target.value})} aria-invalid={Boolean(errors.yearsExperience)||undefined} aria-describedby={describedBy("yearsExperience")}/>{error("yearsExperience")}</label><label>Office address <span>(optional)</span><textarea name="officeAddress" rows={3} maxLength={500} value={agentDraft.officeAddress} onChange={event=>setAgentDraft({...agentDraft,officeAddress:event.target.value})}/></label><label>Working towns <span>(up to five, comma separated)</span><input name="workingTowns" maxLength={300} placeholder="Ongole, Chirala" value={agentDraft.workingTowns} onChange={event=>setAgentDraft({...agentDraft,workingTowns:event.target.value})}/></label><label>Specialisations <span>(comma separated)</span><input name="specializations" maxLength={500} placeholder="Residential, Commercial, Agricultural" value={agentDraft.specializations} onChange={event=>setAgentDraft({...agentDraft,specializations:event.target.value})}/></label><label>Professional introduction <span>(optional)</span><textarea name="about" rows={4} maxLength={1500} value={agentDraft.about} onChange={event=>setAgentDraft({...agentDraft,about:event.target.value})}/></label><p className="form-note">Your profile remains private and pending until administrator review and approval.</p></fieldset>}
    ...
    <fieldset className="wide"><legend>Communication consent</legend><p className="form-note">Choose how OngoleProperty.com may contact you about your account and property requirements.</p><label className="check-label"><input type="checkbox" name="consentEmail" value="accepted" defaultChecked/>Email</label><label className="check-label"><input type="checkbox" name="consentWhatsapp" value="accepted" defaultChecked/>WhatsApp</label><label className="check-label"><input type="checkbox" name="consentSms" value="accepted" defaultChecked/>SMS</label><label className="check-label"><input type="checkbox" name="consentPhone" value="accepted" defaultChecked/>Phone Call</label></fieldset>
    <CaptchaWidget/>
