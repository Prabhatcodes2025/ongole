...
export function RegistrationForm({ initialAccountType, returnTo }: { initialAccountType: string; returnTo: string }) {
  ...
  return <form action="/api/auth/register" method="post">
    ...
    <label>Password<input required type="password" name="password" autoComplete="new-password" minLength={8} maxLength={200} pattern="(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9])(?=.*[^A-Za-z0-9]).{8,}" aria-describedby="password-help"/><small id="password-help">Use at least 8 characters with uppercase, lowercase, a number and a special character.</small></label>
    <CaptchaWidget/>
