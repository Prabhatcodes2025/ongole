...
export async function DashboardShell({children,title,description,actions,variant="owner",breadcrumbs=[]}:{children:ReactNode;title:string;description?:string;actions?:ReactNode;variant?:"owner"|"admin";breadcrumbs?:{label:string;href?:string}[]}){
  ...
  const nav=(()=>{const items=[...(variant==="admin"?adminNav:ownerNav)];const hasAdminAccess=permissions.some(permission=>["users.read","properties.read","pg.read","agents.read","enquiries.read","audit.read","settings.manage"].includes(permission));if(variant==="owner"&&hasAdminAccess)items.unshift({href:"/admin",label:"Administration",icon:<LayoutDashboard/>});return items.filter((item)=>!item.permission||permissions.includes(item.permission))})();
