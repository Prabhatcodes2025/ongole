...
export function ListingResults({properties,basePath="/properties"}:{properties:PublicProperty[];basePath?:string}){const params=useSearchParams();const router=useRouter();const urlView=params.get("view");const [view,setView]=useState<"grid"|"list">(()=>{if(urlView==="list")return "list";if(typeof window!=="undefined"&&!urlView&&sessionStorage.getItem("property-view")==="list")return "list";return "grid";});
  const change=(next:"grid"|"list")=>{setView(next);sessionStorage.setItem("property-view",next);const query=new URLSearchParams(params.toString());query.set("view",next);router.replace(`${basePath}?${query}`,{scroll:false});};
