...
export function PgFields({defaults}:{defaults?:Record<string,unknown>}){
  ...
  return <>
    ...
    <label className="wide">Description <span>(maximum 250 words; no contact details, links or advertisements)</span><textarea required name="description" minLength={40} rows={6} defaultValue={String(defaults?.description||"")}/></label>
    <label className="wide">Street address<textarea required name="address_line" rows={3} defaultValue={String(defaults?.address_line||"")}/></label>
    ...
    <label className="wide">YouTube video URLs only (one per line)<textarea name="video_urls" rows={3} defaultValue={(defaults?.video_urls as string[]|undefined)?.join("\n")||String(defaults?.video_urls||"")}/></label>
...
export function PublicPgPostingForm(){
  ...
  function continueToAccount(event:FormEvent<HTMLFormElement>){event.preventDefault();const form=event.currentTarget;if(!form.reportValidity())return;setBusy(true);setMessage("");try{sessionStorage.setItem(STORAGE_KEY,JSON.stringify(valuesFrom(form)));window.location.assign("/login?returnTo=%2Fdashboard%2Fpg%2Fnew%3Frestore%3D1")}catch{setMessage("Your details could not be stored in this browser. Please try again.");setBusy(false)}}
...
export function DashboardPgDraftForm(){
  ...
  useEffect(()=>{try{const stored=sessionStorage.getItem(STORAGE_KEY);setDefaults(stored?JSON.parse(stored):{})}catch{setDefaults({})}},[]);
